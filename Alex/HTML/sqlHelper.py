...
#this SHOULD work to get the bar chart    
def get_bar(self, Country, State):
    # Build the WHERE clause based on the selected country and state
    if Country == 'All':
        where_clause = "1=1"
    else:
        where_clause = f"Country = '{Country}'"
        if State != 'All':
            where_clause += f" AND State = '{State}'"
